...
def simulate_tf_deep_test():
    ...
    print("Visualization saved to 'data/08_digiton_tf_deep_test.png'")
    
    # Save Audio
    # 1. Noisy (What you hear)
    wavfile.write('data/08_digiton_tf_deep_noisy.wav', fs, (sig_noisy[:fs*5]/np.max(np.abs(sig_noisy))*32767).astype(np.int16))
    
    # 2. Recovered (The Ghost)
    wavfile.write('data/08_digiton_tf_deep_recovered.wav', fs, (sig_recovered/np.max(np.abs(sig_recovered))*32767).astype(np.int16))
    print("Audio saved.")
